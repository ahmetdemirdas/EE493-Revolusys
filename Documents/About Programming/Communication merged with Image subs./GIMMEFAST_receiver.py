...
intended_distance = 5
sd_type = 'experimental'
environment = 'light'
terminal_distance = [150, 100, 50]
lap_time = [1.95 , 1.39 , 0.92]
avg_speed = [61.5, 50.0, 22.0]
...
while 1:
  if i == 1:
    print("Communication speed is 19200 bps")
    print("It takes 0.536 seconds to transmit one packet")
    print("Distance between the terminals is ", terminal_distance[index], " cm" )
    print("Average speed is ", avg_speed[index], " cm/sec")
    print("The time it takes to travel is ",lap_time[index], " seconds")
    print(' ')
    print("The vehicle arrives to the terminal for the first time") 
    print('The vehicle is sending the request')
    request = input(">> ")
    print("The vehicle sent the request")
    request = request + '\n'
    ser.write(bytes(request,encoding ='utf-8'))
    i = i + 1
    total_time = total_time + lap_time[index]
    actual_distance = sensor_real_distance(intended_distance, sd_type)
    error_rate = communication_error_rate(environment, actual_distance, 'constant')
  ...
  while ser.in_waiting:
    ...
    x = communication_error_creator_concentrated(x,error_rate)
    x = bytes(x)
    #print('Lenght of image bytes',len(x))
    checksum_result = checksum(data=x, sum=check)
    print('Checksum result is ', checksum_result)
    ...
    if action == 'CHU': # if there will be other packets coming
      ...
      if checksum_result == 0:
        ...
        if request == 'FIF\n': # if it was the last packet, then the photo is complete
            ...
            print("Total time spent is: ", total_time)
            print("DONE!")
        else:
            print(' ')
            if i == 2:
              print("The vehicle arrives to the terminal for the second time")
              total_time = total_time + lap_time[index]*2
            elif i == 3:
              print("The vehicle arrives to the terminal for the third time")
              total_time = total_time + lap_time[index]*2
            elif i == 4:
              print("The vehicle arrives to the terminal for the fourth time")
              total_time = total_time + lap_time[index]*2
            elif i == 5:
              print("The vehicle arrives to the terminal for the fifth time")
              total_time = total_time + lap_time[index]
